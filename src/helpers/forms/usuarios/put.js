import { actualizarDocente } from '@/actions/usuario/actualizarDocente'

export async function put (prevFormData, formData, toast, updatePrevFormData, setPending, updateAvatar, updateJWT) {
  const oldData = { ...prevFormData, nacionalidad: prevFormData.nacionalidad.value, idEspecialidad: prevFormData.idEspecialidad.value }
  const newData = { ...formData, nacionalidad: formData.nacionalidad.value, idEspecialidad: formData.idEspecialidad.value }
  const changedData = {}
  ...
  try {
    ...
    const response = await actualizarDocente(Array.from(newFormData.entries()))
